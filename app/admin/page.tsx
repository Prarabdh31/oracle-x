...
interface Team {
  ...
  // Updated to strictly match your database column names
...
// --- SUB-COMPONENT: Reusable Stat Input ---
function StatInput({ label, value, onChange, step = "1", highlight = false }: any) {
  return (
    <div className="flex flex-col items-center">
      <label className={`text-[8px] font-black tracking-widest mb-1 ${highlight ? 'text-red-500 drop-shadow-[0_0_5px_rgba(239,68,68,0.5)]' : 'text-gray-500'}`}>
        {label}
      </label>
      <input
        type="number"
        ...
        value={value ?? ""}
        onChange={onChange}
        className={`w-full text-center bg-black/60 border rounded-md text-sm py-1.5 focus:outline-none focus:border-red-500 font-bold transition-colors ${
          highlight ? 'text-red-500 border-red-900/50 bg-red-950/20' : 'text-white border-white/10'
        }`}
      />
    </div>
  );
...
// --- SUB-COMPONENT: Draggable Admin Team Row ---
function SortableAdminRow({
  ...
  moveUp: () => void;
  moveDown: () => void;
  onStatChange: (id: number, field: keyof Team, value: string) => void;
}) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`relative flex flex-col p-3 rounded-2xl border transition-all duration-200 ${
        isDragging 
          ? "border-red-500 scale-105 bg-red-950/40 shadow-[0_0_20px_rgba(239,68,68,0.4)] opacity-95" 
          : "border-white/10 bg-black/40 hover:bg-gray-900 hover:border-white/20"
      }`}
    >
      {/* Top Half: Drag Handle & Team Identity */}
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center gap-4 flex-grow" {...attributes} {...listeners}>
          <div className="text-gray-500 cursor-grab active:cursor-grabbing px-1 hover:text-red-400 transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8h16M4 16h16" />
            </svg>
          </div>
          <span className="text-xl font-black w-6 text-center text-gray-400">
            {index + 1}
          </span>
          {team.logo_url && <img src={team.logo_url} alt={team.short_name} className="w-8 h-8 object-contain drop-shadow-md" />}
          <p className="font-bold text-white text-lg tracking-wide">{team.short_name}</p>
        </div>

        {/* Manual Arrow Fallback */}
      ...
      {/* Bottom Half: Live Stats Data Grid */}
...
export default function AdminPage() {
  ...
  const checkAdminAndFetchData = async () => {
    ...
    if (teamsData) {
      ...
      if (standingsData && standingsData.length > 0) {
        mergedTeams = mergedTeams.map(team => {
          ...
          return { 
            ...team, 
            current_position: standing?.current_position || 99,
            matches_played: standing?.matches_played || 0,
            wins: standing?.wins || 0,
            losses: standing?.losses || 0,
            no_results: standing?.no_results || 0,
            net_run_rate: standing?.net_run_rate || 0,
    ...
    if (knockoutsData) {
      setFinalist1Id(knockoutsData.finalist_1_id || "");
      setFinalist2Id(knockoutsData.finalist_2_id || "");
      setWinnerId(knockoutsData.winner_id || "");
    }
  ...
  const handleStatChange = (id: number, field: keyof Team, value: string) => {
  ...
  const handleUpdateEverything = async () => {
    ...
    try {
      ...
      const standingsToSave = teams.map((team, index) => ({
        ...
        no_results: Number(team.no_results) || 0,
        net_run_rate: Number(team.net_run_rate) || 0,
      ...
      const knockoutData = {
        id: 1,
        finalist_1_id: finalist1Id === "" ? null : Number(finalist1Id),
        finalist_2_id: finalist2Id === "" ? null : Number(finalist2Id),
        winner_id: winnerId === "" ? null : Number(winnerId),
      };
      ...
      const settingsData = {
        id: 1,
        tournament_deadline: new Date(deadline).toISOString(),
        unlock_team_view: unlockTeamView
      };

      const { error: settingsError } = await supabase.from("system_settings").upsert(settingsData, { onConflict: 'id' });
      if (settingsError) throw settingsError;

      alert("DATABASE OVERRIDDEN. Live stats and payloads successfully published.");
  ...
  const getTeamLogo = (id: number | "") => {
    if (id === "") return null;
    return teams.find(t => t.id === Number(id))?.logo_url || null;
  };
      ...
      <div className="max-w-md mx-auto p-4 relative z-10">
          ...
          <div className="space-y-5">
            <div>
              ...
              <input 
                ...
                className="w-full bg-gray-900/80 border border-white/10 text-white font-bold rounded-xl p-3 text-sm focus:outline-none focus:border-red-500 focus:bg-red-950/10 transition-colors"
              />
            </div>
            ...
            <div className="flex items-center justify-between bg-gray-900/80 border border-white/10 p-4 rounded-xl">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  ...
                  checked={unlockTeamView}
                  onChange={() => setUnlockTeamView(!unlockTeamView)}
                />
          ...
          <div className="space-y-4">
            <div className="bg-gray-900/80 border border-white/10 rounded-xl p-3 flex items-center gap-3">
              <div className="w-10 h-10 bg-black rounded-lg border border-gray-800 flex items-center justify-center shrink-0">
                {getTeamLogo(finalist1Id) ? (
                  <img src={getTeamLogo(finalist1Id)!} className="w-7 h-7 object-contain" />
                ) : (
                  <span className="text-gray-600 text-[10px] font-black">F1</span>
                )}
              </div>
              <div className="flex-grow">
                ...
                <select 
                  value={finalist1Id} 
                  onChange={(e) => setFinalist1Id(e.target.value === "" ? "" : Number(e.target.value))}
                ...
                >
                  ...
                  {teams.map(team => (
                    <option key={team.id} value={team.id} className="bg-gray-900">{team.name}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div className="bg-gray-900/80 border border-white/10 rounded-xl p-3 flex items-center gap-3">
              <div className="w-10 h-10 bg-black rounded-lg border border-gray-800 flex items-center justify-center shrink-0">
                {getTeamLogo(finalist2Id) ? (
                  <img src={getTeamLogo(finalist2Id)!} className="w-7 h-7 object-contain" />
                ) : (
                  <span className="text-gray-600 text-[10px] font-black">F2</span>
                )}
              </div>
              <div className="flex-grow">
                ...
                <select 
                  value={finalist2Id} 
                  onChange={(e) => setFinalist2Id(e.target.value === "" ? "" : Number(e.target.value))}
                  className="w-full bg-transparent text-white font-bold text-sm focus:outline-none"
                >
                  <option value="" className="bg-gray-900">-- Awaiting Result --</option>
                  {teams.map(team => (
            ...
            <div className="mt-6 pt-4 border-t border-white/5">
              <div className="bg-yellow-900/10 border border-yellow-500/30 rounded-xl p-3 flex items-center gap-3 shadow-[0_0_15px_rgba(234,179,8,0.1)]">
                <div className="w-12 h-12 bg-black rounded-xl border-2 border-yellow-500/50 flex items-center justify-center shrink-0 shadow-[0_0_10px_rgba(234,179,8,0.2)]">
                  {getTeamLogo(winnerId) ? (
                    <img src={getTeamLogo(winnerId)!} className="w-9 h-9 object-contain drop-shadow-md" />
                  ) : (
                    <svg className="w-5 h-5 text-yellow-600/50" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 2a1 1 0 011 1v1h1a1 1 0 010 2H6v1a1 1 0 01-2 0V6H3a1 1 0 010-2h1V3a1 1 0 011-1zm0 10a1 1 0 011 1v1h1a1 1 0 110 2H6v1a1 1 0 11-2 0v-1H3a1 1 0 110-2h1v-1a1 1 0 011-1zM12 2a1 1 0 01.967.744L14.146 7.2 17.5 9.134a1 1 0 010 1.732l-3.354 1.935-1.18 4.455a1 1 0 01-1.933 0L9.854 12.8 6.5 10.866a1 1 0 010-1.732l3.354-1.935 1.18-4.455A1 1 0 0112 2z" clipRule="evenodd" /></svg>
                  )}
                </div>
                <div className="flex-grow">
                  ...
                  <select 
                    ...
                    className="w-full bg-transparent text-yellow-500 font-black text-base focus:outline-none"
                  >
                    <option value="" className="bg-gray-900 text-white">-- Crown Winner --</option>
                    {teams.filter(t => t.id === finalist1Id || t.id === finalist2Id).map(team => (
                      <option key={team.id} value={team.id} className="bg-gray-900 text-white">{team.name}</option>
                    ))}
                  </select>
                </div>
              </div>
        ...
        <div className="fixed bottom-28 left-0 right-0 p-4 bg-gradient-to-t from-[#050814] via-[#050814]/90 to-transparent z-40 pb-safe">
          <button 
            ...
            disabled={saving}
            className={`w-full max-w-md mx-auto flex items-center justify-center py-4 font-black tracking-widest rounded-2xl transition-all shadow-2xl relative overflow-hidden group ${
              saving 
                ? "bg-red-900 text-red-300 border border-red-700 cursor-not-allowed" 
                : "bg-gradient-to-r from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white border border-red-500/50 hover:shadow-[0_0_30px_rgba(220,38,38,0.5)]"
            }`}
          >
            {saving ? (
              <>
              ...
              <>
                <span className="relative z-10">PUBLISH TO LIVE SERVER</span>
