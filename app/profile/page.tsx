...
export default function ProfilePage() {
  ...
  const [battlefieldTotal, setBattlefieldTotal] = useState(0);
  const [leagueTotal, setLeagueTotal] = useState(0);
  
  const [predictedTeams, setPredictedTeams] = useState<TeamBlock[]>([]);
  const [showAvatarModal, setShowAvatarModal] = useState(false);
  const [selectedTeam, setSelectedTeam] = useState<TeamBlock | null>(null); // New state for Drilldown Modal
  ...
  const fetchProfileAndData = async () => {
    ...
    if (teamsData && leagueData && leagueData.length > 0) {
      ...
      const blocks: TeamBlock[] = leagueData.map(pred => {
        ...
        return {
          ...
          breakdown: {
            liveRank,
            predictedRank: pred.predicted_position,
            basePoints: liveRank !== null ? 100 : 0,
            penalty,
            subtotal,
            multiplier: pred.multiplier,
            multipliedTotal,
      ...
      setBattlefieldTotal(blocks.reduce((sum, b) => sum + b.breakdown.battlefieldBonus, 0));
      setLeagueTotal(blocks.reduce((sum, b) => sum + b.breakdown.finalRowPoints, 0));
    }
  ...
  const handleSaveProfile = async () => {
    setSaving(true);
    const toastId = toast.loading("Updating System Identity...");
    try {
      const { data: { session } } = await supabase.auth.getSession();
    ...
    <div className="min-h-screen bg-[#050814] text-white pb-32 relative overflow-hidden">
      ...
      <div className="max-w-md mx-auto p-4 relative z-10">
        <div className="flex flex-col items-center mt-6 mb-10">
          <div onClick={() => setShowAvatarModal(true)} className="relative group cursor-pointer mb-4">
            ...
            <img src={selectedAvatar} alt="Avatar" className="relative w-28 h-28 rounded-full border-4 border-cyan-500 object-cover bg-gray-900 shadow-[0_0_25px_rgba(6,182,212,0.5)] z-10" />
          ...
          <div className="mt-4 bg-gradient-to-r from-gray-900 to-black border border-gray-800 px-6 py-2 rounded-2xl shadow-xl flex flex-col items-center gap-1">
            <div className="flex items-center gap-2">
            ...
            {battlefieldTotal > 0 && (
              <div className="flex items-center gap-2 text-[10px] font-bold tracking-wider">
                <span className="text-gray-500">League: {leagueTotal}</span>
                <span className="text-gray-700">|</span>
                <span className="text-amber-400">&#9876; Battlefield: {battlefieldTotal}</span>
              </div>
            )}
          </div>
        </div>
              ...
              {predictedTeams.map((pred) => {
                ...
                return (
                  <div 
                    ...
                    className="bg-black/40 border border-white/5 backdrop-blur-md rounded-xl p-2 flex flex-col items-center justify-between relative overflow-hidden shadow-lg aspect-square cursor-pointer hover:border-cyan-500/50 hover:bg-white/5 transition-all active:scale-95"
                  >
                    ...
                    <div className="w-full text-center mt-1">
                      <div className="bg-gray-900/80 border-t border-white/5 py-1 w-full flex flex-col items-center">
                        ...
                        <span className="text-[10px] font-black text-cyan-400 drop-shadow-[0_0_5px_rgba(6,182,212,0.5)]">{pred.pointsEarned + pred.breakdown.battlefieldBonus} PTS</span>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <div className="relative">
                <input type="text" id="firstName" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder=" " className="peer w-full px-4 pt-5 pb-2 bg-gray-900/50 border border-white/10 rounded-xl text-white font-bold focus:outline-none focus:border-cyan-500 focus:bg-cyan-950/10 transition-all placeholder-transparent" />
                <label htmlFor="firstName" className="absolute left-4 top-1 text-[10px] font-bold text-gray-500 uppercase tracking-widest transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-600 peer-focus:top-1 peer-focus:text-[10px] peer-focus:text-cyan-500 pointer-events-none">First Name</label>
              </div>
              <div className="relative">
                <input type="text" id="lastName" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder=" " className="peer w-full px-4 pt-5 pb-2 bg-gray-900/50 border border-white/10 rounded-xl text-white font-bold focus:outline-none focus:border-cyan-500 focus:bg-cyan-950/10 transition-all placeholder-transparent" />
                <label htmlFor="lastName" className="absolute left-4 top-1 text-[10px] font-bold text-gray-500 uppercase tracking-widest transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-600 peer-focus:top-1 peer-focus:text-[10px] peer-focus:text-cyan-500 pointer-events-none">Last Name</label>
              </div>
            </div>
            <div className="relative">
              <input type="text" id="entryName" value={entryName} onChange={(e) => setEntryName(e.target.value)} placeholder=" " className="peer w-full px-4 pt-5 pb-2 bg-gray-900/50 border border-white/10 rounded-xl text-white font-bold focus:outline-none focus:border-cyan-500 focus:bg-cyan-950/10 transition-all placeholder-transparent" />
              <label htmlFor="entryName" className="absolute left-4 top-1 text-[10px] font-bold text-gray-500 uppercase tracking-widest transition-all peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-sm peer-placeholder-shown:text-gray-600 peer-focus:top-1 peer-focus:text-[10px] peer-focus:text-cyan-500 pointer-events-none">Team Display Name</label>
            </div>
            <button onClick={handleSaveProfile} disabled={saving} className="w-full py-4 mt-2 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-black tracking-widest rounded-xl shadow-[0_0_20px_rgba(6,182,212,0.2)] hover:shadow-[0_0_25px_rgba(6,182,212,0.4)] transition-all disabled:opacity-50 relative overflow-hidden group">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in" onClick={() => setShowAvatarModal(false)}>
          <div className="bg-[#0f1423] border border-cyan-500/30 rounded-3xl p-6 max-w-sm w-full max-h-[80vh] flex flex-col shadow-[0_0_50px_rgba(6,182,212,0.15)] relative" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowAvatarModal(false)} className="text-gray-500 hover:text-white bg-gray-900 rounded-full p-1"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
              <div className="grid grid-cols-4 gap-3">
                {PREMADE_AVATARS.map((avatar, index) => {
                  ...
                  return (
                    <button key={index} onClick={() => { setSelectedAvatar(avatar); setShowAvatarModal(false); }} className={`relative aspect-square rounded-2xl overflow-hidden transition-all duration-200 border-2 ${isSelected ? "border-cyan-500 scale-110 shadow-[0_0_15px_rgba(6,182,212,0.6)] z-10" : "border-gray-800 opacity-60 hover:opacity-100 hover:border-gray-600 bg-black/50"}`}>
                      <img src={avatar} alt={`Avatar`} className="w-full h-full object-cover" />
                      {isSelected && <div className="absolute inset-0 bg-cyan-500/20 ring-inset ring-2 ring-cyan-500/50 rounded-xl pointer-events-none"></div>}
                    </button>
                  );
                })}
              </div>
            </div>
            <div className="mt-6 pt-4 border-t border-white/10 text-center text-[10px] text-gray-500 font-black tracking-widest uppercase">{PREMADE_AVATARS.length} Entities Available</div>
          </div>
        </div>
      )}
